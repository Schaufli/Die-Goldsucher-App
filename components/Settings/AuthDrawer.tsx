...
import { X, LogIn, LogOut, Mail, Lock, UserPlus, User as UserIcon, CheckCircle, Camera, Trash2, Save, Edit2, Map } from 'lucide-react';
...
export const AuthDrawer: React.FC<AuthDrawerProps> = ({ isOpen, onClose, user, forceOpen = false }) => {
  ...
  const formTitle = user ? 'Dein Profil' : (
    mode === 'login' ? 'Anmelden' : 
  ...
  if (forceOpen) {
    return (
      <div className="fixed inset-0 z-[1000] flex flex-col items-center justify-center bg-brand-bg px-4 overflow-y-auto">
        <div className="flex flex-col items-center mb-4 mt-4 shrink-0">
          <div className="w-16 h-16 rounded-2xl bg-brand-gold flex items-center justify-center mb-2 shadow-lg">
            <Map className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-xl font-bold text-brand-text">Die Goldsucher App</h1>
          <p className="text-brand-textSec text-xs mt-0.5">Melde dich an, um loszulegen</p>
        </div>
        <div className="bg-white w-full max-w-md rounded-2xl p-5 shadow-2xl mb-4 shrink-0">
          <h2 className="text-lg font-bold text-brand-text mb-3">{formTitle}</h2>
          {messagesBlock}
          <div className="flex flex-col gap-4">
            {mode === 'verify' ? (
              <div className="flex flex-col items-center text-center py-6 gap-6">
                ...
                <Button variant="primary" onClick={() => setMode('login')} className="w-full py-3.5 text-lg">
              ...
              <>
                <form onSubmit={handleEmailAuth} className="flex flex-col gap-4">
                  {mode === 'login' && (
                    <>
                      <button type="button" onClick={handleGoogleLogin} disabled={loading} className="w-full py-3 px-4 bg-white border border-gray-300 rounded-xl text-gray-700 font-medium hover:bg-gray-50 transition-colors flex items-center justify-center gap-3 shadow-sm active:scale-[0.98]">
                      ...
                      <div className="relative my-2">
                        ...
                        <div className="relative flex justify-center text-sm"><span className="px-4 bg-white text-gray-500">Oder mit E-Mail</span></div>
                      </div>
                    </>
                  )}
                  {mode === 'register' && (
                    <>
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} required className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand-gold focus:border-transparent transition-all outline-none" placeholder="Dein Vorname" />
                        </div>
                      </div>
                      <button type="button" onClick={handleGoogleRegister} disabled={loading} className="w-full py-3 px-4 bg-white border border-gray-300 rounded-xl text-gray-700 font-medium hover:bg-gray-50 transition-colors flex items-center justify-center gap-3 shadow-sm active:scale-[0.98] mt-2">
                        <svg className="w-5 h-5" viewBox="0 0 24 24">
                          <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                          <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                          <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand-gold focus:border-transparent transition-all outline-none" placeholder="mail@beispiel.de" />
                    </div>
                  </div>
                  {mode !== 'forgot' && (
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand-gold focus:border-transparent transition-all outline-none" placeholder="••••••••" />
                      </div>
                    </div>
                  )}
                  {mode === 'register' && (
                    <div>
                      ...
                      <div className="relative">
                        <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
                        <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required minLength={6} className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand-gold focus:border-transparent transition-all outline-none" placeholder="••••••••" />
                      </div>
                    </div>
                  )}
                  {mode === 'login' && (
                    <div className="flex justify-end">
                      <button type="button" onClick={() => setMode('forgot')} className="text-sm text-brand-gold hover:text-yellow-700 font-medium">Passwort vergessen?</button>
                    </div>
                  )}
                ...
                <div className="mt-4 text-center">
                  {mode === 'login' ? (
                    <p className="text-sm text-gray-600">Noch kein Konto?{' '}<button type="button" onClick={() => setMode('register')} className="text-brand-gold font-bold hover:underline">Jetzt registrieren</button></p>
                  ) : (
                    <p className="text-sm text-gray-600">{mode === 'register' ? 'Bereits ein Konto?' : 'Zurück zur'}{' '}<button type="button" onClick={() => setMode('login')} className="text-brand-gold font-bold hover:underline">Anmeldung</button></p>
                  )}
                </div>
              </>
            )}
    ...
    <div className="fixed inset-0 z-[1000] flex items-end sm:items-center justify-center pointer-events-none">
      ...
      <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-2xl p-6 shadow-2xl transform transition-transform animate-slide-up pointer-events-auto max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-brand-text">{formTitle}</h2>
